...
class CONFIG:
    ...
    CACHE_DIR = "./cache_npz"
    NUM_SUBJECTS_TO_LOAD = 118
    N_FOLDS = 2
    TARGET_LENGTH_LSTM = 1500
    NUM_CLASSES = 6
    SLEEP_STAGE_LABELS = ["Wake", "N1", "N2", "N3", "REM", "Noise"]  # thêm lớp thứ 6
...
def build_model(input_shape, num_classes):
    """
    Builds a Bidirectional GRU model with an Attention layer.
    """
    model = tf.keras.Sequential([
        Input(shape=input_shape),
        Bidirectional(LSTM(128, return_sequences=True, kernel_regularizer=l2(CONFIG.L2_REG))),
        BatchNormalization(),
        Dropout(CONFIG.DROPOUT_RATE),
        Bidirectional(LSTM(64, return_sequences=True, kernel_regularizer=l2(CONFIG.L2_REG))),
        BatchNormalization(),
        Dropout(CONFIG.DROPOUT_RATE),
        AttentionLayer(),
        Dense(64, activation='relu', kernel_regularizer=l2(CONFIG.L2_REG)),
        BatchNormalization(),
...
def make_balanced_dataset(X_list, y_list, batch_size, num_classes=6, augment_stages=None):
    ...
    class_indices = {cls: np.where(all_y == cls)[0] for cls in unique_classes if len(np.where(all_y==cls)[0])>0}

    def generator():
        while True:
            batch_x, batch_y = [], []
            while len(batch_x) < batch_size:
                cls = np.random.choice(list(class_indices.keys()))
...
def evaluate_and_report_on_subject(model, subject_id, config):
    ...
    for idx in range(X_full.shape[0]):
        try:
            x_raw = X_full[idx]
            len_in, len_out = x_raw.shape[0], config.TARGET_LENGTH_LSTM
...
if __name__ == "__main__":
    ...
    for i, (train_idx, test_idx) in enumerate(gkf.split(range(len(unique_subjects)), groups=unique_subjects)):
        ...
        input_shape = (CONFIG.TARGET_LENGTH_LSTM, X_train_list[0].shape[-1])
        ...
        # Callbacks
        ckpt_path = f"best_model_fold_{i+1}.keras"
        ...
        cbs = [
            ModelCheckpoint(f"temp_model_fold_{i+1}.keras", monitor='val_loss', mode='min', save_best_only=False, verbose=0),
            clean_f1_callback,
            SpeedLogger(),
        ]
    ...
    os.makedirs("final_reports/clean", exist_ok=True)
    os.makedirs("final_reports/full", exist_ok=True)

    # --- Load cache nếu có ---
    cache_path = "results/training_cache.pkl"
    ...
    # Lưu báo cáo CLEAN
    with open("final_reports/clean/report_clean.txt", "w", encoding='utf-8') as f:
    ...
    # Lưu báo cáo FULL
    with open("final_reports/full/report_full.txt", "w", encoding='utf-8') as f:
    ...
    # ================== Lưu summary báo cáo ==================
    with open("final_reports/summary_report.txt", "w", encoding='utf-8') as f:
        ...
        f.write("- final_reports/clean/report_clean.txt\n")
        f.write("- final_reports/full/report_full.txt\n")
    ...
    print("\n" + noise_impact_summary)
    print("\n✅ Báo cáo chi tiết đã được lưu vào thư mục 'final_reports/clean' và 'final_reports/full'.")
    print("✅ Báo cáo tóm tắt đã được lưu vào 'final_reports/summary_report.txt'.")
    ...
    plot_confusion_matrix(cm_clean, CONFIG.SLEEP_STAGE_LABELS[:-1], "final_reports/clean/confusion_matrix_clean.png", "CLEAN Confusion Matrix (5 lớp)")
    plot_confusion_matrix(cm_full, CONFIG.SLEEP_STAGE_LABELS, "final_reports/full/confusion_matrix_full.png", "FULL Confusion Matrix (6 lớp, có Noise)")
    ...
    plot_stage_metrics(y_true_clean_final, y_pred_clean_final, CONFIG.SLEEP_STAGE_LABELS[:-1], range(5), "final_reports/clean/per_stage_metrics_clean.png", "Hiệu suất mô hình theo từng giai đoạn (CLEAN, 5 lớp)")
    plot_stage_metrics(y_true_full, y_pred_full, CONFIG.SLEEP_STAGE_LABELS, range(6), "final_reports/full/per_stage_metrics_full.png", "Hiệu suất mô hình theo từng giai đoạn (FULL, 6 lớp, có Noise)")
    ...
    plot_fold_comparison(f1_clean_folds, kappa_clean_folds, "final_reports/clean/fold_comparison_metrics_clean.png", "So sánh Macro F1 & Kappa qua các Folds (CLEAN)")
    plot_fold_comparison(f1_full_folds, kappa_full_folds, "final_reports/full/fold_comparison_metrics_full.png", "So sánh Macro F1 & Kappa qua các Folds (FULL)")
    ...
    if not best_model_path or not os.path.exists(best_model_path):
        candidates = sorted(glob.glob("best_model_fold_*.keras") + glob.glob("*.keras"))
        if candidates:
    ...
    print(f"✅ Results and plots saved to 'results/' and 'final_reports/' directories")
